...
export const DataProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [rooms, setRooms] = useState<Record<string, RoomData>>(() => {
        ...
        if (savedRoomsStr) {
            try {
                ...
                const mergedRooms = { ...initialRoomsData };
                for (const key in savedRooms) {
                    if (mergedRooms[key]) {
                        // Только цену берем из кэша, остальное (включая картинки) всегда из кода
                        mergedRooms[key] = { ...mergedRooms[key], price: savedRooms[key].price || mergedRooms[key].price };
                    }
                }
                return mergedRooms;
    ...
    const [news, setNews] = useState<NewsItem[]>(() => {
        const savedNews = localStorage.getItem("app_news_v3");
        return savedNews ? JSON.parse(savedNews) : initialNewsData;
    });

    const [calculatorData, setCalculatorData] = useState<CalculatorCategory[]>(() => {
        const savedCalcStr = localStorage.getItem("app_calculator_v3");
        if (savedCalcStr) {
            try {
                const savedCalc = JSON.parse(savedCalcStr);
                return initialCalculatorData.map(cat => {
                    const savedCat = savedCalc.find((c: any) => c.id === cat.id);
                    if (savedCat) {
                        return {
                            ...cat,
                            rooms: cat.rooms.map(room => {
                                const savedRoom = savedCat.rooms?.find((r: any) => r.id === room.id);
                                if (savedRoom) {
                                    return { ...room, priceNight: savedRoom.priceNight || room.priceNight, pricePlace: savedRoom.pricePlace || room.pricePlace };
                                }
                                return room;
                            })
                        };
    ...
    useEffect(() => {
        localStorage.setItem("app_news_v3", JSON.stringify(news));
    }, [news]);

    useEffect(() => {
        localStorage.setItem("app_calculator_v3", JSON.stringify(calculatorData));
    }, [calculatorData]);
